...
def contact_has_replied(driver, contact, last_message, logger):
    """
    ...
    try:
        ...
        try:
            ...
            logger.info(f'Button to close chat not sent!')

    except Exception as e:
        logger.info(f"Phonebot found a replie from the profile: {e}")

    return [has_replied, answer]


def send_message(driver, contact, message, logger):
    """
    ...
    try:
        ...
        # Click on send
        try:
            clickOnElement(driver, findElement(driver, "xpath",
                                               '//*[contains(@aria-label, "Envoyer") or '
                                               'contains(@aria-label, "Send") or '
                                               'contains(@aria-label, "envoyer") or '
                                               'contains(@aria-label, "send")]'))
        except TimeoutException as toe:
            logger.info(f'PhoneBot did not found send button verify xpath selector:\n{toe}')
            # try:
            #     clickOnElement(driver, findElement(driver, "xpath",
            #                                        '//*[contains(@aria-label, "Envoyer un J’aime") or '
            #                                        'contains(@aria-label, "Send a Like")]'))
            #
            # except TimeoutException as toe:
            #     logger.info(f'PhoneBot did not found send button verify xpath selector:\n{toe}')
        ...
        logger.error(f'Fail to send message to {contact["username"]}')
...
def search_new_contacts(p_driver, dataBaseTool, logger, platform, browser, id_task_user, current_username, today, rest):
    """
    ...
    list_groups = getColumnsFromGoogleSheetByURL(p_taskuser["url_list"])
    nb_new_contacts = 0
    counter = 0
    ...
    for group in list_groups:
        ...
        url = f'{group[0]}members' if group[0][-1] == "/" else f'{group[0]}/members'
        p_driver.get(url)
        try:
            # Open members tabs
            see_all_counter = 0
            ...
            for i in range(0, see_all_counter):
                ...
                p_driver.get(url)  # redirect to the members page
                see_all = WebDriverWait(p_driver, 2.5).until(EC.presence_of_all_elements_located((
                    By.XPATH,
                    '//a[contains(@aria-label, "Voir tout") or contains(@aria-label, "See All")]'
                )))
                ...
                time.sleep(2.5)
                new_add_friend = WebDriverWait(p_driver, 5).until(EC.presence_of_all_elements_located((
                    By.XPATH,
                    '//span[@dir="auto"]//a[@role = "link" and contains(@href, "group") and contains(@href, "user") '
                    'and not(contains(@aria-hidden, "true"))] '
                )))
                # Check if the number of contacts it at least = to actions limit, if not scroll to bottom of page
                if len(new_add_friend) < rest:
                    mymodulesteam.ScrollToTheEnd(p_driver)
                    new_add_friend = WebDriverWait(p_driver, 2.5).until(EC.presence_of_all_elements_located((
                        By.XPATH,
                        '//span[@dir="auto"]//a[@role = "link" and contains(@href, "group") and contains(@href, '
                        '"user") and not(contains(@aria-hidden, "true"))] '
                    )))
                ...
                # add urls to list new_add_profile
            ...
            for profile in new_add_profile:
                ...
                try:
                    # Get profile data
                    try:
                        name = findElement(p_driver, 'tag_name', 'h1').text
                    except TimeoutException as toe:
                        name = findElement(p_driver, 'tag_name', 'h2').text

                    contact_profile = [name,
                                       f'https://www.facebook.com/profile.php?id={profile_id}',
                                       group[0]]

                    if not contact_in_database(dataBaseTool, logger, platform, browser, id_task_user,
                                               current_username, contact_profile[0], today):
                        new_contacts.append(contact_profile)
                        counter += 1
                    else:
                        logger.info(f'we have this contact already')
...
# def Browser_Facebook_send_message_group_members(p_browser, p_taskuser_id, current_username, p_quantity_actions, label_log, lock): ...
...
def Browser_Cold_Messaging_Facebook_Group_Members(p_browser, p_taskuser_id, current_username, driver,
    ...
    *************************************************************************
    ...
    if len(messageContacts) != 0:
        ...
        for contact in messageContacts:
            ...
            res = contact_has_replied(driver, contact, last_message, logger)
            ...
            if res[0]:  # la personne a répondu
                ...
                if not \
                        dataBaseTool.modifyLine("contacts", ["replied", "date_update"], [1, today],
                            ...
                            "ok"]:
                    logger.info(
                        "database modify replied failed ! " + getExceptionFormat(response["data"]))
                    return False
    ...
    *************************************************************************
    ...
    if len(messageContacts) != 0:
        ...
        for contact in messageContacts:
            ...
            if not send_message(driver, contact, next_message, logger):
            ...
            if number_of_messages_sent_today >= p_quantity_actions:
                return True
    ...
    *************************************************************************
    ...
    if len(contacts_waiting_for_accept["data"]) >= 1:
        ...
        for contact_waiting_for_accept in contacts_waiting_for_accept:
            logger.info(
                logSectionMessage("check invitation for {} ...".format(contact_waiting_for_accept["username"])))
    ...
    new_contacts = search_new_contacts(driver, dataBaseTool, logger, platform, p_browser, p_taskuser_id,
                                       current_username,
                                       today,
                                       (p_quantity_actions, number_of_messages_sent_today)[number_of_messages_sent_today > 0])
    ...
    for new_contact in new_contacts:  # new_contacts = [[username, url], [username, url], ....]
        ...
        if not dataBaseTool.addLine("actions", ("platform", "type_action", "id_smartphone", "id_social_account",
                                                "id_contact", "date_created", "id_task_user"),
                                    (platform, "scrap", p_browser, current_username, newLineContact["ok"], today,
                                     p_taskuser_id))["ok"]:
            logger.info("add new action \"scrap\" failed !")
            return False
...
Browser_Cold_Messaging_Facebook_Group_Members(p_browser, p_taskuser_id, facebook_username, p_driver,
