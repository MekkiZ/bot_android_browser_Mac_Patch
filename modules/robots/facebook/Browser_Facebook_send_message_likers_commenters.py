...
def send_message(driver, contact, message):
    ...
    try:
        ...
        try:
            ...
            # used to avoid the absecence of send btn
            pass
...
def search_new_contacts(driver, dataBaseTool, logger, platform, browser, id_task_user, current_username, today):
    """
    ...
    """
    new_contacts = []
    task_user = mymodulesteam.GetDetailsTaskUserMysql(id_task_user)
    post_list = [post_row[0] for post_row in get_post_list(task_user['url_list'])]

    bd_profiles = dataBaseTool.getLinesRequestNoDict(f"SELECT url_profile FROM contacts "
                                                     f"where id_task_user = {id_task_user}")["data"]
    tmp = []
    for url in bd_profiles:
        tmp.append(url[0])
    bd_profiles = tmp

    for post in post_list:
        if len(new_contacts) > task_user["daily_limit"]: break
        new_contacts.extend(get_fb_profiles(driver, task_user["daily_limit"], post, bd_profiles))

    return new_contacts


# ============================= get_post_list ========================================
def get_post_list(sheetUrl):
    if sheetUrl is None:
        mymodulesteam.logger.info(f'url spreadsheet not found')
        return False
    return mymodulesteam.GoogleSheetGetValues(mymodulesteam.extract_ss_id_regex(sheetUrl))


# ==================================== Scrape new profiles==============================================================

def get_fb_profiles(driver, limit, post, bd_profiles):
    """
    Get profiles that comment and liked the post
    """
    driver.get(post)
    time.sleep(.5)
    commenters = get_commenters(driver, limit, bd_profiles)

    driver.get(post)
    time.sleep(.5)
    profiles = get_likers(driver, limit, bd_profiles)

    # To avoid duplicated url profiles, we check if it's not already added to profiles array
    for commenter in commenters:
        if commenter not in profiles:
            profiles.append(commenter)

    return profiles


# ======================================================================================================================
def get_commenters(p_driver, limit, bd_profiles):
    """
        Get commenters profiles
    """
    # commentersBlockSelector = ".l9j0dhe7.tkr6xdv7.buofh1pr.eg9m0zos"

    mymodulesteam.ScrollToTheEnd(p_driver)
    # isEnd = False
    profiles = []
    try:
        commenters_profiles = WebDriverWait(p_driver, 5).until(EC.presence_of_all_elements_located((
            By.XPATH, '//*[contains(@aria-label, "Comment")]//a[@tabindex="-1"]'
        )))


        tmp = []
        for commenter_profile in commenters_profiles:
            tmp.append(commenter_profile.get_attribute('href'))
        commenters_profiles = tmp

        i = 0
        for commenter in commenters_profiles:
            # profiles.extend([commenter.get_attribute('href').split("?")[0] for commenters in commenters_profiles])
            if i >= (limit*1.5): break

            if commenter not in bd_profiles:
                p_driver.get(commenter)
                time.sleep(0.5)
                name = get_profile_name(p_driver)
                url = commenter
                profiles.append([name, url])
                i += 1

    except TimeoutException as toe:
        mymodulesteam.logger.error(f'Didn\'t not found any commenters : {toe}')
        return False

    tmp_profiles = []
    for profile in profiles:
        if profile not in tmp_profiles: tmp_profiles.append(profile)

    return tmp_profiles


def get_likers(p_driver, limit, bd_profiles):
    """ Get liker profiles """

    likers_block_selector = ".q5bimw55.otl40fxz"
    isEnd = False
    profiles = []

    try:
        # Open likers popup
        likers_Button = WebDriverWait(p_driver, 10).until(EC.presence_of_element_located
                                                          ((By.XPATH, ("//span[@class='pcp91wgn']"))))
        clickOnElement(p_driver, likers_Button)
        time.sleep(random.uniform(1, 3))

        while not isEnd:
            # find all  likers_profiles
            profiles_block = WebDriverWait(p_driver, 10).until(EC.presence_of_all_elements_located
                                                               ((By.XPATH, ("//div[@class='q9uorilb']/a"))))
            profiles = [profile_element.get_attribute('href') for profile_element in profiles_block]

            isEnd = scroll_down(p_driver, likers_block_selector)
        ...
        mymodulesteam.logger.error(f'Didn\'t not found any likers : {ex}')
        profile = None
    ...
    for profile in profiles:
        ...
        if profile not in bd_profiles:
            p_driver.get(profile)
            time.sleep(0.5)
            name = get_profile_name(p_driver)
            url = profile
            if [name, url] not in tmp_profiles: tmp_profiles.append([name, url])
            i += 1

    return tmp_profiles


def scroll_down(p_driver, selector):
    """A method for scrolling the page."""
    SCROLL_PAUSE_TIME = 1

    # Get scroll height

    last_height = p_driver.execute_script("return document.querySelectorAll('" + selector + "')[0].scrollHeight")

    p_driver.execute_script(
        "document.querySelectorAll('" + selector + "')[0].scrollTo(0, document.querySelectorAll('" + selector + "')[0].scrollHeight);")

    time.sleep(SCROLL_PAUSE_TIME)

    # Calculate new scroll height and compare with last scroll height
    new_height = p_driver.execute_script("return document.querySelectorAll('" + selector + "')[0].scrollHeight")
    time.sleep(2)

    if new_height == last_height:
        mymodulesteam.logger.info('scroll down in the end')
        return True
...
# --------------------------------------------------------------------------------------------------------------------
def get_profile_name(p_driver):
    try:
        name = WebDriverWait(p_driver, 2.5).until(EC.presence_of_element_located
                                                  ((By.TAG_NAME, "h1")))
        name = name.text
        if name == '':
        ...
        try:
            clickOnElement(p_driver,
                           WebDriverWait(p_driver, 2.5).until(EC.presence_of_element_located
                                                              ((By.XPATH, '//*[contains(@aria-label, "Voir") or '
                                                                          'contains(@aria-label, "View")]')))
                           )
            ...
            try:
                # Click on Plus
                clickOnElement(p_driver,
                               WebDriverWait(p_driver, 2.5).
                               until(EC.presence_of_element_located(
                                   (By.XPATH, '//*[contains(@aria-haspopup, "menu") and @role="button"]')))
                               )
                ...
                mymodulesteam.logger.info(f"Profile isn't public, the scrape it's not possible:\n{toe}")
                name = ''
...
# ---------------------------------------------------------------------------------------------------------------------#
def Browser_Cold_Messaging_Likers_Commenters_Fb_post(p_browser, p_taskuser_id, FB_username, p_driver,
                                                     p_quantity_actions, label_log, lock):
    logger = initLogger()
    platform = 'facebook'

    return cold_messaging_all(p_driver, lock, logger, p_browser, platform, p_taskuser_id, FB_username,
                              p_quantity_actions)
...
def cold_messaging_all(driver, lock, logger, p_browser, platform, p_taskuser_id, current_username, p_quantity_limit):
    ...
    *************************************************************************
    ...
    if not response["ok"]:
        ...
        return result, number_of_messages_sent_today
            ...
            if not \
                    dataBaseTool.addLine("actions",
                                         ("platform", "type_action", "message", "id_smartphone", "id_social_account",
                                          "id_contact", "date_created", "id_message", "date_update", "id_task_user"),
                                         (
                                         platform, "send_message", next_message, p_browser, current_username, contact["id"],
                                         today,
...
#
#
# result, counter = Browser_Cold_Messaging_Likers_Commenters_Fb_post(p_browser, p_taskuser, facebook_username, p_driver,
#                                                                    p_quantity_actions, label_log, lock)
# print(f'{result} - {counter}')
#
#
# # profiles = ['https://www.facebook.com/groups/2616981278627207/user/514885352/?__tn__=R*F',
# #             'https://www.facebook.com/groups/2616981278627207/user/100067536910796/?__tn__=-a',
# #             'https://www.facebook.com/groups/2616981278627207/user/100053796430655/?__tn__=-a',
# #             'https://www.facebook.com/groups/2616981278627207/user/100071118560373/?__tn__=-a',
# #             'https://www.facebook.com/groups/2616981278627207/user/100002320222970/?__tn__=-a',
# #             'https://www.facebook.com/groups/2616981278627207/user/100050056671513/?__tn__=-a']
# #
