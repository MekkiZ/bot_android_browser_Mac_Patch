...
def contact_has_replied(driver, contact, last_message):
    ...
    time.sleep(5)
    #on clique sur le bouton d'envoi de message
    sendm = driver.find_element_by_xpath("//*[contains(text(), 'Envoyer')]")
    sendm.click()

    time.sleep(2)
    #on insère le nom du compte qui va recevoir le message 
    search=driver.find_element_by_xpath("//input[@name='queryBox']")
    search.send_keys(contact["username"])
    
    time.sleep(2)
    #on clique sélection le compte 
    search=driver.find_element_by_xpath("//div[@class='-qQT3']")
    search.click()

    time.sleep(2)
    #on clique sur suivant 
    search=driver.find_element_by_xpath("//*[contains(text(), 'Suivant')]")
    search.click()
    
    
    time.sleep(4)
    # On cherche la balise "<span>" qui contient notre message
    my_messages = driver.find_elements_by_xpath("//div[contains(@class, 'eFVJg') and @role='listbox']")
    last_div = my_messages[len(my_messages) - 1]
    last_span = last_div.find_element_by_xpath(".//span")

    following_divs = last_span.find_elements_by_xpath("./following::div[contains(@class, 'JdNBm') and @role='listbox']")

    has_replied = len(following_divs) >= 1

    for following_div in following_divs:
        message = following_div.find_element_by_xpath(".//span").text
        answer += message + " "
...
def send_message(driver, contact, message):
    ...
    try:
        #on se rédirige vers la zone d'envoie de message
        driver.get("https://www.instagram.com/direct/inbox/")
        ...
        time.sleep(2)
        #on écrit le message
        search=driver.find_element_by_xpath("//textarea[@placeholder='Votre message…']")
        automaticSendKeys(search, message)
...
def search_new_contacts(driver, dataBaseTool, logger, platform, browser, id_task_user, current_username, today, rest):
    ...
    """
    list_of_contacts = getColumnsFromGoogleSheetByURL("https://docs.google.com/spreadsheets/d/17mFalDiwdFsULTEPoET_uFszYHzOhsHMcMja0UuD9UM/edit?usp=sharing")
    ...
    for url in list_of_contacts: 
        ...
        likers=driver.find_elements_by_xpath("//a[@class='zV_Nj']")
        
        if len(likers)==1: likers[0].click() #on vérifie le nombre d'élément que nous renvoie le Xpath pour cliquer sur le bon

        if len(likers)==2: likers[1].click() #on vérifie le nombre d'élément que nous renvoie le Xpath pour pouvoir cliquer sur le bon
        ...
        #liste entière des personnes qui ont likers/des personnes qui ont aimé un post
        likers_list= driver.find_element_by_xpath("//div[contains(@style,'hidden auto')]")
        ...
        while lastScrollTop != currentScrollTop and len(news_comptes)<rest: # lastScrollTop -1; currentScrollTop -2; indexOfFirstLiker 0
            ...
            update_likers =driver.find_elements_by_xpath("//a[starts-with(@class,'FPmhX')]")
            ...
            for index in range(0, len(update_likers)):
                ...
                if(compte.text not in news_comptes):
                    if not contact_in_database(dataBaseTool, logger, platform, browser, id_task_user, current_username, update_likers[index].text, today): 
                        news_comptes.append([update_likers[index].text, None])
...
def Browser_Cold_Messaging_Instagram_Likers_and_commenters_of_Instagram_post(p_browser, p_taskuser_id, instagram_username, p_quantity_actions, label_log, lock):
    ...
    time.sleep(2) #pause de 1 seconde avant d'exécuter le reste du code

    #on récupère le champ email/numéro/nom d'utilisateur et on le modifie 
    identifiant=driver.find_element_by_xpath("//form[@id='loginForm']/div//input[@name='username']")
    identifiant.send_keys("TheMouMou_2")

    time.sleep(1)
    #on récupère le champ mot de passe et on le modifie
    motdepasse=driver.find_element_by_xpath("//form[@id='loginForm']/div//input[@name='password']")
    motdepasse.send_keys("1234AZER")

    time.sleep(1)
    #Récupérer le boutton login et clique
    login=driver.find_element_by_xpath("//form[@id='loginForm']//button[@type='submit']/div[.='Connexion']") 
    login.click() 

    time.sleep(10)

    return cold_messaging_all(driver, lock, logger, p_browser, "instagram", p_taskuser_id, instagram_username, p_quantity_actions)
...
def cold_messaging_all(driver, lock, logger, p_browser, platform, p_taskuser_id, current_username, p_quantity_limit):
    ...
    info_task = mymodulesteam.GetDetailsTaskUser(p_taskuser_id)
    ...
    *************************************************************************
    ...
    if len(contacts_waiting_for_accept["data"]) >= 1:
        ...
        for contact_waiting_for_accept in contacts_waiting_for_accept:
            ...
            if contact_has_replied_invitation(driver, contact_waiting_for_accept):
                ...
                logger.info("{} hasn't yet a, p_quantity_actions - number_of_messages_sent_todayccepted ...".format(contact_waiting_for_accept["username"]))
    ...
    for new_contact in new_contacts:  # new_contacts = [[username, url], [username, url], ....]
        ...
        if need_invitation(driver, new_contact[1]):  # On a besoin de lui envoyer une invitation avant de parler
            ...
            if not dataBaseTool.addLine("actions", ("platform", "type_action", "id_smartphone", "id_social_account",
                                             "id_contact", "date_created", "date_update", "id_task_user"),
                (platform, "send_invitation", p_browser, current_username, newLineContact["ok"], today, today,
                           p_taskuser_id))["ok"]:
...
#
# Browser_Cold_Messaging_Instagram_Likers_and_commenters_of_Instagram_post(p_browser_run, p_taskuser_id_run, facebook_username_run, p_quantity_actions_run, label_log_run, lock_run)
